...
public class FNGGramatica {
	
	public static Gramatica g;
	
	public static Gramatica fng(Gramatica gramatica) {
	...
	private static void reordenarColocandoRecursivasNoFim() {
		...
		for(int i = 0; i < g.variaveis.size(); i++) {
			...
			if(v.temRecursaoAEsquerda()) {
				aux.add(v);
				g.variaveis.remove(i);
				i--;
			}
		}
		g.variaveis.addAll(aux);
	}
	
	private static void removerProducoesQueIniciamComVariaveisAnteriores() {
		//Ja pula a primeira
		for(int i = 1; i < g.variaveis.size(); i++) {
			Variavel v = g.variaveis.get(i);
	...
	private static void criarAuxiliaresParaRecursivas() {
	...
	private static boolean trocarVariaveisAEsquerdaPorTerminais() {
		...
		for(Variavel v : g.variaveis) {
			...
			for(int i = 0; i < v.getProducoes().size(); i++) {
				...
				if(Character.isUpperCase(prod.charAt(0))) {
					...
					Variavel substituicao = g.buscarVariavel(prod.charAt(0));
	...
	private static Variavel criarAuxiliarParaRecursao(List<Variavel> variaveisAuxiliares, String producaoRecursiva) {
		char letra;
		outer:
